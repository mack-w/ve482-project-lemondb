...
std::pair<std::string, bool> ComplexQuery::initCondition(const Table &table) noexcept  {
...
std::pair<std::string, int> ComplexQuery::initCondition_(const Table &table) noexcept  {
  const std::unordered_map<std::string, int> opmap{
      {">", '>'}, {"<", '<'}, {"=", '='}, {">=", 'g'}, {"<=", 'l'},
  };
  std::pair<std::string, int> result = {"", 0};
  if (condition.empty()) return std::make_pair("",-1);
  for (auto &cond : condition) {
    if (cond.field == "KEY") {
      // if (cond.op != "=") {
      //   throw IllFormedQueryCondition("Can only compare equivalence on KEY");
...
bool ComplexQuery::evalCondition(const Table::Object &object)  noexcept {
  bool ret = true;
...
bool ComplexQuery::testKeyCondition (
    Table &table, const std::function<void(bool, Table::Object::Ptr &&)>& function) noexcept  {
