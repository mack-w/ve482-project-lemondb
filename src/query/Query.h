...
public:
  ...
  virtual QueryResult::Ptr execute(int threadsNum) noexcept = 0 ;
...
public:
  ...
  std::string toString()  noexcept  override { return "QUERY = NOOP"; }
};
...
public:
   ...
   */
  std::pair<std::string, bool> initCondition(const Table &table) noexcept;
  
  std::pair<std::string, int> initCondition_(const Table &table) noexcept;
  /**
   ...
   * @return
   */
  bool evalCondition(const Table::Object &object) noexcept;
  ...
  testKeyCondition(Table &table,
                   const std::function<void(bool, Table::Object::Ptr &&)>& function) noexcept;

  ComplexQuery(std::string targetTable, std::vector<std::string> operands,
               std::vector<QueryCondition> condition) noexcept
