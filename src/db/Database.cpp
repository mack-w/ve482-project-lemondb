...
Table &Database::loadTableFromStream(std::istream &is, const std::string& source) {
  ...
  auto fileSize = std::filesystem::file_size(std::filesystem::path(source));
  ...
  while (std::getline(is, line)) {
    ...
    table->reserve(fileSize/sizeof(tuple));
...
Table &Database::copyTable(const std::string &srcTableName, const std::string &destTableName){
  auto &db = Database::getInstance();
  auto &srcTable = db[srcTableName];
  auto table = std::make_unique<Table>(destTableName, srcTable);
  return db.registerTable(std::move(table));
}
